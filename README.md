...
* ⏩ **“호환 문서 — 배치가 변경될 수 있습니다” 확인 창 자동 계속**:
  DOCX·RTF 등으로 저장할 때 한글이 띄우는 「저장을 계속할까요?」 창에 자동으로 「계속」하여 대량 변환이 멈추지 않습니다. 앱이 띄운 한글 프로세스에만 적용되며 옵션에서 끌 수 있습니다.
* 🔄 **메모리 누수 제로 (200건 단위 프로세스 자동 재순환)**:
  대량 문서 변환 시 한컴 프로세스가 느려지거나 멈추는 현상을 방지하기 위해, 200건 변환마다 백그라운드 한글 COM 인스턴스를 무결하게 재순환(Recycle)합니다.
* 💾 **철저한 원본 보호 및 안전 백업**:
  변환 작업 전 원본 문서를 `backup/` 폴더에 자동으로 안전하게 복사하며, 중복 파일 덮어쓰기 방지 및 백업 수량 순환 보관(파일당 1~100개)을 지원합니다. 「기존 파일 덮어쓰기」를 켜도 **기존 `.hwp`/`.hwpx` 원본 문서는 절대 덮어쓰지 않고** 새 이름으로 저장합니다.
* 🎯 **듀얼 PDF 엔진 탑재**:
...
> [!NOTE]
> * 입력 문서가 이미 목표 형식인 경우(예: HWP로 변환할 때 `.hwp` 입력)는 **자동으로 건너뜁니다.**
> * 같은 이름의 결과 파일이 이미 있으면 기본적으로 `이름 (1).pdf`처럼 **새 이름으로 저장**하며, 「기존 파일 덮어쓰기」를 켜면 덮어씁니다(단, `.hwp`/`.hwpx` 원본 문서는 항상 보호).
> * 이미지(PNG/JPG/BMP/GIF)는 한글이 페이지마다 `문서001.png`, `문서002.png` 형태로 저장합니다.
> * 암호가 걸린 문서나 손상된 파일은 변환 단계에서 **실패**로 집계되고, 결과 리포트에 암호·보호 안내가 함께 표시됩니다.
...
### Q6. DOCX 변환 중 “호환 문서 — 문서 내용의 배치가 변경될 수 있습니다. 저장을 계속할까요?” 창이 떠서 멈춥니다.
> **A:** 한글이 호환 형식(DOCX, RTF 등) 저장 시 띄우는 확인 창입니다. HwpMate는 옵션 **「호환 형식 저장 확인 창 「계속」 자동 선택」**(기본 켜짐)으로 앱이 띄운 한글 프로세스의 이 창에 자동으로 「계속」합니다. 결과 리포트에 자동 처리 횟수가 표시되며, 변환된 문서는 원본과 배치가 다를 수 있으니 확인해 주세요. CLI에서 끄려면 `--no-auto-continue`를 사용합니다.

### Q7. 변환 중 한글 프로그램을 만져도 되나요?
...
### 4. 품질 검증 및 테스트 실행
HwpMate는 신뢰성을 위해 220개 이상의 단위/통합 테스트와 엄격한 타입 체킹을 유지합니다. 실제 한글 COM 동작은 `tools/hwp_com_smoke.py`와 `HWP_COM_SMOKE_TEST_CHECKLIST.md`로 별도 검증합니다.
```powershell
# 정적 타입 검사
pyright .

...
- **감사(`PROJECT_AUDIT.md`) 이슈 조치:**
  ...
  - 원본 보호: 덮어쓰기 + HWP/HWPX 변환이 같은 stem의 원본 문서를 교체하던 문제 수정 (`allocate_output_path` 원본 보호 + 사전 점검 경고).
  - 백업 정리: `report.hwp` 정리가 `report_1.hwp` 백업을 지우던 문제 수정 (정확 패턴 + 파일명 타임스탬프 정렬).
  - CLI: GUI와 같은 `task_runner` 사용으로 `--retry`·기본 백업 적용, 건너뜀 집계, 빈 폴더 오류 처리, 입력 확장자 검증, 단일 인스턴스 잠금, `--report`, `--no-auto-continue` 추가.
  - 업데이트: 헬퍼 부모 대기 무력화(`os.kill(pid, 0)`) 수정, 교체 재시도, 상태 명시(`UpdateApplyError.status`), 교체 실패 시 백업 정리·고유 백업명, 롤백 후 재실행, 스테이징 헬퍼 정리, 변환 중 적용 보류, 시작 확인 1회.
  - 재순환: 초기화 3회 재시도, 실패 시 남은 작업 즉시 실패 처리 및 결과 경고.
- **한글 2022(12.0.0.4605) 실제 COM 점검으로 발견·수정:**
  - 이미지(PNG/JPG/BMP/GIF) 변환이 `{stem}001.png` 명명 때문에 전부 실패로 판정되던 문제 수정.
  - ODT가 `SaveAs(..., "ODT")` False로 실패 → `ODF` 우선 + `ODT` 대체 후보.
  - `cleanup()`의 `Clear(3)`이 열린 원본 문서를 저장할 수 있어 `Clear(1)`로 변경.
  - DOCX/RTF 저장 시 「호환 문서 — 배치가 변경될 수 있습니다」 창에서 무한 대기 → 소유 PID 한정 자동 계속(설정/CLI 옵션).
  - 강제 종료 직후 재연결 실패 시 고아 한글 프로세스 정리, 성공 후 문서 닫기 예외가 성공을 실패로 뒤집지 않도록 보호.
  - HTML 본문 이미지(`PIC*.png`)를 산출물로 기록, 형제 문서(`report (1)`) 산출물을 보조 산출물로 오인하지 않도록 매칭 축소.
- **검증:** `pytest` 222 passed, `pyright .` 0 errors, 실제 한글 COM 스모크 16/16 성공, GUI 워커 E2E·CLI E2E PASS.

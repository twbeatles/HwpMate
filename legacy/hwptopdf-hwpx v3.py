...
class HwpAutomation(Protocol):
    ...
    def SaveAs(self, path: str, format_name: str) -> Any: ...
...
class HWPConverter:
    ...
    def initialize(self):
        ...
        for progid in HWP_PROGIDS:
            try:
                self.hwp = cast(HwpAutomation, win32com.client.Dispatch(progid))
    ...
    def convert_file(self, input_path, output_path, format_type="PDF"):
        ...
        try:
            ...
            hwp.Open(input_str, "HWP", "forceopen:true")
    ...
    def cleanup(self):
        """정리"""
        hwp = self.hwp
        if hwp and self.is_initialized:
            try:
